...
> [!NOTE]
> **Google News系プロファイルの位置づけ（2026-07-16、`model_watch`プロファイルでの試験運用より）**
> - Google News RSSは**タイトルとリンクのみ**で、本文サマリーは取得できない（`summary`には本文ではなくGoogleのリダイレクトタグしか入らない）。リンク先もJSリダイレクトのため、プログラムからの本文スクレイピングは不可。
> - そのためGoogle News系プロファイル（`model_watch`等）は、**記事本文を伴う完成した記事のソースにはならない**。あくまで「見出しレベルで候補ネタを拾う一次探索」として使うもの。
> - 正しい運用フロー: ① プロファイルでRSS収集 → ② 見出しから使えそうなネタを人間が選ぶ → ③ 該当記事の本文を人間がブラウザ等で取得（ペイウォール越えも人間側で対応）→ ④ 本文をWKFLとのチャットに渡し、通常の「ピックアップ・ニュース」（ルートB）の3段階プロセス（Dossier→Article→Podcast）に載せる。
> - キーワードにありがちなノイズ（例: `quota` が「牛肉輸入クオータ」や「女性クオータ法案」等の無関係記事まで拾ってしまう事象）について: **収集後に無関係なものを間引く前提であれば、多少ノイズの多いキーワードを使うこと自体は問題ない**。間引いてもなお無関係な結果が多すぎる場合にのみ、そのキーワードを絞る・具体化する（例: `quota` → `usage quota`, `API quota`）という判断でよい。
